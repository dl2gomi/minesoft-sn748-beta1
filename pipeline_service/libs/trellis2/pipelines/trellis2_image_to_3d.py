...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def decode_latent(
        ...
        # NOTE:
        # Decoding multiple sparse samples in one pass can produce invalid voxel/vertex
        # alignment in downstream flexible dual-grid conversion for some backends.
        # Decode each sample independently to keep coords/features consistent.
    ...
    def inject_sampler_multi_image(
        ...
        if mode == 'stochastic':
            ...
            def _new_inference_model(self, model, x_t, t, cond, **kwargs):
                ...
                # Expand cond to match x_t batch size (num_samples may be > 1)
                cond_i = cond[cond_idx:cond_idx + 1].expand(x_t.shape[0], *[-1] * (cond.ndim - 1))
                # Also expand neg_cond to match x_t batch size if present
